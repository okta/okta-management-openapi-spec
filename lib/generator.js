...
function buildOperations(spec) {
  ...
  function buildOperation(pathName, methodName, method) {
    ...
    const bodySchema = _.get(method, 'requestBody.content["application/json"].schema');
    if (bodySchema) {
      operation.bodyModel = getModelName(bodySchema['$ref']);
    }
    // Determine the return type
    const success = _.get(method, 'responses["200"].content["application/json"].schema')
      || _.get(method, 'responses["201"].content["application/json"].schema');
    if (success) {
  ...
  function buildOperationObject(pathName, methodName, method) {
    ...
    return Object.assign({
      ...
      consumes: util.getInputMediaTypes(method),
      produces: util.getOutputMediaTypes(method),
    }, _.pick(method, [
      'operationId',
      'description',
      'summary',
