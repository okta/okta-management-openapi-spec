...
util.ensureReadOnlyProperties = spec => {
  ...
  for (let [modelName, model] of Object.entries(spec.components.schemas)) {
...
util.ensureConsistentStructure = spec => {
  ...
  spec.components.schemas = JSON.parse(stringify(spec.components.schemas));
};
...
util.overrideLinksAndEmbedded = (spec) => {
  ...
  for (let model of Object.values(spec.components.schemas)) {
  ...
  if (spec.components.schemas) {
    delete spec.components.schemas.Link;
    delete spec.components.schemas.LinksUnion;
    delete spec.components.schemas.EmbeddedObject;
  }
};
...
util.handlebarsHelpers = {
  ...
  util.removeUnusedModels = (spec) =>{
    const definedModelNames = Object.keys(spec.components.schemas);
    ...
    _.cloneDeepWith(spec, (value) => {
      if (typeof value === 'string' && value.includes('#/components/schemas')) {
        referencedModelNames.add(value.split('/')[3]);
      }
    })
    // console.log(referencedModelNames);
    ...
    if(definedModelNames.length !== Object.keys(spec.components.schemas).length) {
      util.removeUnusedModels(spec);
    }
  }

util.getInputMediaTypes = (method) => {
  const content = _.get(method, 'requestBody.content');
  return content ? Object.keys(content) : [];
};

util.getOutputMediaTypes = (method) => {
  if (method.responses) {
    const mediaTypes = new Set();
    for (const [statusCode, response] of Object.entries(method.responses)) {
      if (response.content) {
        Object.keys(response.content)
          .forEach(mediaType => mediaTypes.add(mediaType));
      }
    }
