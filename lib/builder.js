...
builder.build = ({spec}) => {
  ...
  if (!_.has(spec, 'components.schemas')) {
    _.set(spec, 'components.schemas', {});
  }
        ...
        util.forEachMapEntry(operation.responses, function(statusCode, response){
          ...
          if (response.schema) {
            if(response.schema.hasOwnProperty('$ref')) {
              ...
              response.schema['$ref'] = "#/components/schemas/" + modelName;
              ...
              response.schema.items['$ref'] = "#/components/schemas/" + modelName;
            }
          }
        });
      }
...
builder.addModels = ({spec}) => {
  ...
  util.forEachMapEntry(spec.components.schemas, function(modelName, model) {
    ...
    spec.components.schemas[modelName] = model;
  });
};
